...
int Rresult_cols =  Right.cols - templ.cols + 1;
int Rresult_rows = Right.rows - templ.rows + 1;

static OwlCorrel OWL;
OWL.ResultLeft.create(result_rows, result_cols,  CV_32FC1 );
OWL.ResultRight.create(Rresult_rows, Rresult_cols, CV_32FC1 );

/// Do the Matching and Normalize
int match_method = 5; /// CV_TM_CCOEFF_NORMED;
matchTemplate( Left, templ, OWL.ResultLeft, match_method );
matchTemplate( Right, templ, OWL.ResultRight, match_method );
...
minMaxLoc( OWL.ResultLeft, &minVal, &maxVal, &minLoc, &maxLoc, Mat() );
minMaxLoc( OWL.ResultRight, &minValR, &maxValR, &minLocR, &maxLocR, Mat() );
...
{
  OWL.MatchLeft = minLoc;
  OWL.MatchRight = minLocR;
}

else
{
 OWL.MatchLeft = maxLoc;
 OWL.MatchRight = maxLocR;
}
